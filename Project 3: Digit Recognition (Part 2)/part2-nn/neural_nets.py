...
class NeuralNetwork():
    ...
    def train(self, x1, x2, y):
        ...
        output = self.hidden_to_output_weights.dot(hidden_layer_activation) # scalar
        activated_output = output_layer_activation(output)  # scalar

        ### Backpropagation ###

        # Compute gradients
        output_layer_error = -(y - activated_output)*output_layer_activation_derivative(output) # scalar
        df1 = np.vectorize(rectified_linear_unit_derivative)
        hidden_layer_error = np.multiply(self.hidden_to_output_weights.transpose(),df1(hidden_layer_weighted_input)) * output_layer_error # TODO (3 by 1 matrix)

        bias_gradients = hidden_layer_error 
        hidden_to_output_weight_gradients = np.multiply(output_layer_error, hidden_layer_activation).T  # 1 by 3
        input_to_hidden_weight_gradients = bias_gradients.dot(input_values.T) # 3 by 2
